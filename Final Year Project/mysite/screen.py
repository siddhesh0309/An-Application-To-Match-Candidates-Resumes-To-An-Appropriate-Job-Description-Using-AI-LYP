...
def res(resumes_data, job_data):
    ...
    # # ranking process
    #
    # for index, file in enumerate(Resume_Vector):
    #     samples = file
    #     print("---------------------")
    #     print(index)
    #     print(Ordered_list_Resume[0])
    #     name = Ordered_list_Resume[index]
    ...
    #     print(score)
    #     #print(name)
    #     result_arr[index] = {'name': name, 'score': score}
    #
    ...
    # return result_arr
    # Assuming some parts of the code are not provided for context

    # Ensure lengths match or handle different lengths
    if len(Ordered_list_Resume) <= len(Resume_Vector):
        for index, file in enumerate(Resume_Vector):
            samples = file
            print("---------------------")
            print(index)
            print(Ordered_list_Resume[0])
            if index < len(Ordered_list_Resume):  # Check index range
                name = Ordered_list_Resume[index]
                neigh = NearestNeighbors(n_neighbors=1)
                neigh.fit(samples)

                # Assuming Job_Desc is defined elsewhere
                score = neigh.kneighbors(Job_Desc)[0][0][0]
                print(score)
                result_arr[index] = {'name': name, 'score': score}
        ...
        print("Lengths of Ordered_list_Resume and Resume_Vector don't match.")
        print(Ordered_list_Resume)
        print(Resume_Vector)
        # You might want to log this discrepancy or handle it according to your logic
